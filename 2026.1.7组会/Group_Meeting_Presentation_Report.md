...
## 📅 本周工作摘要
1.  **数据集构建 (Rationale)**：明确了构建 `xd_dataset` 的核心动机与对下游任务的意义。
2.  **模型机理探索 (Analysis)**：完成了对 Qwen3-VL-2B 模型 Attention Map 的全层扫描与可视化分析。
3.  **发现与挑战 (Insights)**：揭示了 Transformer 内部注意力头（Attention Head）的“分工机制”与“输入依赖性”。

---

## 1. 为什么我们要制作 `xd_dataset`？
*(针对上周师兄提出的疑问，补充立项依据)*

目前通用的多模态模型（如 Qwen-VL 原生权重）虽然在“通用描述”上表现优秀，但在处理我们特定的**异常检测/细粒度动作分析**任务时存在明显的短板。我制作 `xd_dataset` 的核心目的有三点：

1.  **对齐任务范式 (Task Alignment)**：
    *   原生模型的训练数据多为静态图文对（Captioning），缺乏对“时间维度”的敏感性。
    *   本数据集将连续视频帧与时间戳事件（Annotations）结合，旨在**迫使模型建立【视觉特征 -> 时间定位】的映射能力**，而不仅仅是识别画面里有什么。
...
## 2. Qwen3-VL Attention 机制深度探索

本周我重点研究了：**“模型到底有没有看懂由图生文的过程？”**
...
我发现并非所有 Layer 都在关注图像内容。
*   **浅层 (L0-L5)**：关注纹理和边缘。
*   **中层 (L8-L20)**：**语义理解的核心区域**。我们找到了如 `Layer 8 Head 7` 这样的“黄金头”，它能够自动定位到画面中的主体。
*   **深层 (L21-L27)**：信息高度抽象，关注点往往转移到了特殊 Token 上（Sink Tokens），不适合直接可视化。
...
**结论**：同一个 Attention Head 的功能是固定的（探测特征），但其**激活状态高度依赖于输入分布**。这意味着我们不能简单地寻找“万能头”，而需要寻找“鲁棒头”（信噪比高的头）。
...
3.  **架构融合探索 (Architecture Integration - V-JEPA)**：
    *   **引入背景 (What is V-JEPA?)**：参考 Meta FAIR 提出的 **V-JEPA (Video Joint Embedding Predictive Architecture)**。它的核心理念是“不做画家，做思想家”——即**不重建像素，而是预测潜变量特征 (Latent Representations)**。
    *   **为何引入？**：目前的生成式模型（如 Qwen-VL）容易受像素级噪声（光照、纹理）干扰。而 V-JEPA 通过掩码预测任务建立了对物理世界（如物体恒常性、运动规律）的鲁棒理解，且计算极其高效。

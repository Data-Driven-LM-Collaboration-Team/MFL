...
[查看原图](https://github.com/Data-Driven-LM-Collaboration-Team/MFL/blob/main/assets/main-figure.png) · [直接打开图片](https://github.com/Data-Driven-LM-Collaboration-Team/MFL/raw/refs/heads/main/assets/main-figure.png)

# 四阶段训练

训练数据沿用之前的 **613 数据集（`20260613_data`）**，保持源视频级训练、验证与评测隔离。已有事件时间段、检测框和 tracker ID 作为候选标注；人物可见掩码、持续身份、可见性及人物事件区间需补齐并复核。人物归属与跟踪只使用具有有效人物标注的片段。
...
在 613 上，先用旧轨迹定位待标注片段，再核对同一人物的连续可见掩码、遮挡和进出画面，修正 ID 切换，无法确认时截断轨迹。旧检测框不能直接充当人物掩码，未经复核的 tracker ID 不直接作为可靠身份真值。
...
613 的事件时间段需绑定到具体人物及其参与区间，才能生成对象时间级标签；不能把视频级异常标签分配给画面中的所有人。
